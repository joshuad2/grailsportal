 ...
 */
    ...
    def show = {
        ...
        if(!${propertyName}) {
			...
			render (model:[${propertyName}:${propertyName}],view:'show') 
			}
    }
    ...
    def update = {
        ...
        if(${propertyName}) {
            ...
            if(!${propertyName}.hasErrors() && ${propertyName}.save()) {
				...
				flash.message = "${className} \${params.id} has Errors on update Please try again"
				render(action:show,id:${propertyName}.id)
            }
        }
        else {
            flash.message = "${className} not found with id \${params.id}"
    ...
    def save = {
        ...
        if(!${propertyName}.hasErrors() && ${propertyName}.save()) {
			...
			flash.message = "${className} \${${propertyName}.id} error"
			render(action:show,id:${propertyName}.id)
        }
    }
}
