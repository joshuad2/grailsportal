 ...
 */
    ...
    def show = {
        ...
        if(!paymentInstance) {
			...
			render (model:[paymentInstance:paymentInstance],view:'show') 
			}
    }
    ...
    def update = {
        ...
        if(paymentInstance) {
            ...
            if(!paymentInstance.hasErrors() && paymentInstance.save()) {
				...
				flash.message = "Payment ${params.id} has Errors on update Please try again"
				render(action:show,id:paymentInstance.id)
            }
        }
        else {
            flash.message = "Payment not found with id ${params.id}"
    ...
    def save = {
        ...
        if(!paymentInstance.hasErrors() && paymentInstance.save()) {
			...
			flash.message = "Payment ${paymentInstance.id} error"
			render(action:show,id:paymentInstance.id)
        }
    }
}
